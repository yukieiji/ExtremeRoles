...
namespace ExtremeRoles.Test.Performance
{
    public static class PerformanceTestRunner
    {
        ...
        public static IEnumerator RunComparison(IReadOnlyList<ITestStep> testSteps)
        {
            var logger = ExtremeRolesTestPlugin.Instance.Log;
            var results = new Dictionary<string, List<(long time, long memory)>>();
            foreach (var step in testSteps)
            {
                results[step.GetType().Name] = new List<(long, long)>();
            }
            ...
            for (int i = 0; i < Iterations; i++)
            {
                foreach (var step in testSteps)
                {
                    ...
                    results[step.GetType().Name].Add((stopwatch.ElapsedMilliseconds, memoryAfter - memoryBefore));
                }
            }
        ...
        private static void logResults(ManualLogSource logger, Dictionary<string, List<(long time, long memory)>> results)
        {
            ...
            foreach (var result in results)
            {
                ...
                var medianTime = (times[Iterations / 2 - 1] + times[Iterations / 2]) / 2.0;
                var maxTime = times.Last();

                var avgMem = memories.Average();
                var medianMem = (memories[Iterations / 2 - 1] + memories[Iterations / 2]) / 2.0;
                var maxMem = memories.Last();
