...
EXTREMERORLS_OUT_FILE = os.path.join(WORKING_DIR, "ExtremeRoles", "Resources", "JsonData", "Language")
...
def is_require_update(new_json : Any, output_file : str) -> bool:
...
def export_to_json(xlsx_data:Any, output_file: str) -> None:

    if not is_require_update(xlsx_data, output_file):
      return
...
def xlsx_to_new_json(file_name : str, output_file : str) -> None:
  wb = readxl(file_name)
    ...
    for i in range(2, row + 1):
      ...
      for j in range(2, col + 1):
          ...
          if type(cell_data) != str:
            continue
          lang = sheat.index(1, j)
          if not (lang in xlsx_data):
            xlsx_data[lang] = {}

          xlsx_data[lang][key] = cell_data.replace("\r", "").replace("_x000D_", "").replace("\\n", "\n")

  for lang, data in xlsx_data.items():
    export_to_json(data, os.path.join(output_file, f'{lang}.json'))
...
def xlsx_to_json(file_name : str, output_file : str) -> None:
  ...
  export_to_json(xlsx_data, output_file)

def main() -> None:
  xlsx_to_new_json(EXTREMERORLS_IN_FILE, EXTREMERORLS_OUT_FILE)
