...
![AmongUs](https://img.shields.io/badge/AmongUs-v2021.12.15～v17.2.0(v2026.2.17)-green)
...
# Extreme Roles
主な特徴として以下があります<br>
- 第三陣営「ニュートラル」、第四陣営「リベラル」と幽霊役職の追加
- **100個以上のユニークな役職**の追加
...
- Extreme Roles v2026.1.7.0時点、今後も追加予定
...
|  クルー  |  インポスター  | ニュートラル | リベラル |
| ---- | ---- | ---- | --- |
|  スペシャルクルー  |  スペシャルインポスター  | アリス | リーダー |
|  シェリフ  |  エボルバー  | ジャッカル | |
|  メンテナー  |  キャリアー  | サイドキック | |
|  ニート  |  サイコキラー  | タスクマスター | | 
...
|  エクソシスト  | ハイジャッカー | ナイト  | |
|  ロウワー  | タイムブレーカー | ポーン  | |
|  C.E.O  |　スカベンジャー | ヴィジランテ  | |
|  エコー  | ボクサー | ワルガキ  | |
|  インポスター  | レジスレイター| トレイター  | |
|  マーリン  | アサシン | | |
|  ヒーロー  | シェアーズ |  | |
|  捜査官  |  ヴィラン | | |
|  助手  | | | |
...
* 「ラバーズ」、「サポーター」、「ゲッサー」、「ムーバー」、「アクセラレーター」、「スケーター」と「バーター」はオプション設定によりインポスターにもなります。(デフォルトはクルー)
* 「サイドキック」と「見習い捜査官」、「サーヴァント」、「ドール」、「ヤードバード」、「ロォーブレーカー」、「キメラ」はゲーム開始時にはアサインされず、条件を満たした時にアサインされます
...
* 「リーダー」は「リベラル」が割り当てられる時、必ず出現する特殊な役職です
* 「リベラル」の通常役職は「ドーヴ」と「ミリタント」です
...
- サーヴァント: 復活処理中にクイーンが死亡すると勝ち目がない状態で復活する可能性がある不具合
  - 影響度: 極小
    - 現状一切報告を受けていない不具合(コード上のロジックエラーで確認、以下の条件を全て満たす必要がある) 
      1. 「リザレクター」か「C.E.O」をサーヴァント化している
      2. 復活処理中にキルクールが開けている
      3. 「クイーン」がキルされる
- ヤンデレ: キルボタンを持つサブチーム役職が想い人の場合、想い人との特殊勝利が発生しない不具合
  - 影響度: 極小
    - 現状一切報告を受けていない不具合(コード上のロジックエラーで確認、以下の条件を全て満たす必要がある)
      1. サブチーム役職と「ヤンデレ」が設定されている
      2. 「想い人」にサブチーム役職が割り当てられている
      3. メインチームとなる役職が死亡している(存在している場合)
      4. 想い人との特殊勝利条件を満たす
  - 対応チケット: [#736](https://github.com/yukieiji/ExtremeRoles/issues/736)
- メリー: 陣の一部が隠れる特定条件下において活性化後も陣を使用することができない
  - 影響度: 小
    - 変な場所に陣を置くことで**出れなくなる**だけであるため
  - 対応策: 「メリー」の陣全体が見えるような場所に設置する
  - 対応チケット: [#355](https://github.com/yukieiji/ExtremeRoles/issues/355) 
- バーター: 「ランダムキャスリング」が特殊会議中に使えてしまう不具合
  - 影響度: 極小
    - 基本的に特殊会議の投票はMOD側が制御しているので表示だけの問題
    - 投票先が変わっているように見えても正しく動いているはずです
- バーター: 「ランダムキャスリング」の対象に死亡者が含まれている問題
  - 影響度: 小
    - 最悪想定でも死亡者が再追放されるだけであるといえばだけ
    - この設定をしている時点で票をかき混ぜたいという意図があるだけなのでかき混ぜたいという意図はクリアしているので影響度は少ないと判断
- バーター: 「ランダムキャスリング」が1回で何度も使えてしまう不具合
  - 影響度: 中
    - 実行順と同時実行の問題からユーザーによっては実行順が異なり、結果が変わっているように見えてしまう
    - 会議自体はホストが中心で動作しているがアニメーション等はクライアント側で制御しているため発生してしまう問題
- バーター: 「キャスリング」先のプレイヤーが会議中に死亡するとそのスワップが消えない不具合
  - 影響度: 中
    - 最悪想定でも死亡者が再追放されるだけであるといえばだけ
    - だが、その先が死亡するのであればスワップ自体も行われないのが想定されている挙動であるため(そこは運)
...
| ---- | ---- |
|  v17.0.0(v2025.9.9)/v17.0.1(v2025.10.14)/v17.1.0(v2025.11.18)/v17.2.0(v2026.2.17)  | v2025.9.10.0 ～ v2026.1.7 |
