...
## リリーススケジュール
- Extreme Roles v2.1.1.0:
   - 5月下旬頃リリース予定
   - 「より「Extreme」なExtreme 「Roles」を」プロジェクト第二弾、春の小規模アップデート「新世代」
   - v2.1.0.0で大幅に拡張された役職実装機能等を用いて作り上げた、今までのExtreme Rolesにはない「新世代」の役職(コンビ役職ではない)を3つ(各陣営1つずつ)を実装
- Extreme Roles v2.2.0.0:
   - 6月上旬頃リリース予定
   - 「より「Extreme」なExtreme 「Roles」を」プロジェクト第三弾、初夏の大型アップデート「幽霊役職」
   - AmongUsのMOD史上初死亡したあとの「幽霊」がアサインされる役職「幽霊役職」(バニラの守護天使みたいなもの)の実装を可能にする拡張を行い、いくつか「幽霊役職」を実装します
- Extreme Roles v3.0.0.0:
  - 6月中旬頃リリース予定
  - Extreme Roles互換性アップデート「サブマージド」
  - 5月頭にリリースされた海底MODマップこと「サブマージド」に対応、それに伴い複数のMODの導入と対応を可能にする拡張を行います
- Extreme Roles vx.x.x.0:
   - 時期不明
   - 中国語対応アップデート(現在作業中)
...
- シェリフとタスクに関連する様々なオプション (v2.1.1.0開発版にて実装済み)
