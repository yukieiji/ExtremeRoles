...
- レスポンス
  - `AuOptionCategoryDto[]` (JSON)
    ...
    - `Options` `AuOptionDto[]` : オプションのリスト
      ...
      - `Value` any : 現在の値。`ValueType` が `RoleBase` の場合は `AuRoleOption` オブジェクト
...
- パラメーター
  - `VanillaOptionPutRequest` (Body JSON)
...
- レスポンス
  - `UpdatedOptions` (JSON)
    - `UpdatedCategory` `ExRCategoryDto?` : 更新されたカテゴリの情報（詳細は [ExRCategoryDto](#exrcategorydto) を参照）
    - `ChainUpdatedOption` `CategoryOptionDto[]` : 連動して更新されたオプションのリスト
      - `Id` int : カテゴリ ID
      - `Options` `ExROptionDto[]` : 更新されたオプションのリスト（詳細は [ExROptionDto](#exroptiondto) を参照）
...
<a id="exroptiondto"></a>
  - `ExROptionDto` (JSON)
    ...
    - `RangeMeta` `IOptionRangeMeta` : 値の範囲に関するメタ情報
      - `Type` string : 範囲の型
...
- パラメーター
  - `GetTranslationRequest` (Body JSON)
...
- パラメーター (Body JSON)
  - `GetTranslationRequest[]` : [GetTranslationRequest](#post-autranslation) オブジェクトの配列

- レスポンス
  - `GetTranslationResponse[]` (JSON) : 各要素の詳細は [GetTranslationResponse](#post-autranslation) を参照
...
- レスポンス
  - `RoleAssignFilterDto` (JSON)
    - `FilterSet` object : フィルタ ID (GUID) をキーとした `RoleAssignFilterSetDto` オブジェクトのマップ
    - `FilterRoleId` int[] : フィルタに使用されている全役職の ID リスト
    - `NormalRoleId` object : ID (int) からバニラ役職名 (string) へのマッピング
    - `CombinationId` object : ID (int) から組み合わせ役職名 (string) へのマッピング
    - `GhostRoleId` object : ID (int) からゴースト役職名 (string) へのマッピング

  - `RoleAssignFilterSetDto` (JSON)
    - `AssignNum` int : このフィルタに割り当てられる人数
    - `FilterNormalId` object : このフィルタに含まれるバニラ役職の ID (int) と役職名 (string) のマップ
    - `FilterCombinationId` object : このフィルタに含まれる組み合わせ役職の ID (int) と役職名 (string) のマップ
    - `FilterGhostRoleId` object : このフィルタに含まれるゴースト役職の ID (int) と役職名 (string) のマップ
...
- パラメーター
  - `DeltRoleAssignFilter` (Body JSON)
