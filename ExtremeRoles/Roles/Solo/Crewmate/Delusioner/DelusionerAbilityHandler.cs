...
public sealed class DelusionerAbilityHandler(
	DelusionerCounterSystem? system,
	DelusionerStatusModel status,
	DelusionerRole role) : IAbility, IKilledFrom
{
	public ExtremeAbilityButton? Button { get; set; }
	private readonly DelusionerCounterSystem? system = system;
	...
	public void ReduceCounterNum()
	{
		if (this.Button is not null &&
			...
			Button.State == AbilityState.Ready &&
			Button.Behavior is ICountBehavior countBehavior &&
	...
	public bool TryKilledFrom(PlayerControl rolePlayer, PlayerControl fromPlayer)
    {
        ...
        List<PlayerControl> allPlayer = Player.GetAllPlayerInRange(
            rolePlayer, this.role, status.Range);
        ...
        {
            UseAbilityTo(rolePlayer, target, false, targets, false);
        }
		...
		bool isRemoveSystem = true)
    {
            ...
            status.DeflectDamagePenaltyMod < 1.0f)
        {
            ...
            this.Button.Behavior.SetCoolTime(status.CurCoolTime);
        }

		if (isRemoveSystem && this.system is not null)
		{
			this.system.Remove();
		}
