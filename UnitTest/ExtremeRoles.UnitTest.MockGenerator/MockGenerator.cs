...
public class MockGenerator : IIncrementalGenerator
{
    ...
    private (string, string) GenerateInterfaceSource(INamedTypeSymbol classSymbol)
    {
        ...
        foreach (var member in classSymbol.GetMembers().OfType<IMethodSymbol>())
        {
            ...
            {
                string parameters = string.Join(", ", member.Parameters.Select(p => $"{p.Type.ToDisplayString()} {p.Name}"));
                string returnType = member.ReturnsVoid ? "void" : member.ReturnType.ToDisplayString();
        ...
        foreach (var member in classSymbol.GetMembers())
        {
            if (member.IsStatic)
            {
                ...
                {
                    string mockMethodName = $"Mock_{staticMethod.Name}";
                    var parameters = staticMethod.Parameters.Select(p => $"{p.Type.ToDisplayString()} {p.Name}").ToList();
					string parameterNames = string.Join(", ", staticMethod.Parameters.Select(p => p.Name));
                    string returnType = staticMethod.ReturnsVoid ? "void" : staticMethod.ReturnType.ToDisplayString();

                    string actionOrFuncType;
                    if (staticMethod.ReturnsVoid)
                    {
                        actionOrFuncType = parameters.Count != 0 ? $"System.Action<{string.Join(", ", staticMethod.Parameters.Select(p => p.Type.ToDisplayString()))}>" : "System.Action";
                    }
                    else
                    {
