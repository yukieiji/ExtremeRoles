...
## 開発時のルール
 - テストおよびビルドの確認は `run_tests` スクリプト (`run_tests.sh` / `run_tests.ps1`) を実行して行うこと (`dotnet` コマンド単体では動作しません)
 - 変更を加えた箇所のテストは必ず実装すること
 - テストは意味のあるテストを追加すること (固定値の検証のみなどは無意味)
 - AmongUs側のコードは削除されているため `NotImplementedException` が発生します。そのため `Assert.Throws<NotImplementedException>` でテストを片付けることを禁止し、Moqを使用してAmongUs側のコードをモック化すること
 - slnファイル及びプロジェクトファイルの変更を固く禁じます
 - 日本語で返信をすること
 - コードはコードスタイルに従うこと
