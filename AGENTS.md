...
## 応答・コミュニケーション規則
- **謙遜・クッション言葉・無駄な挨拶の絶対禁止**:
  - 以下のような文言、およびそれに類する表現（丁寧な伺い立て・前置き・状況確認・謝罪・感謝）の使用を一切禁止する。
    - 禁止例: 「ご指摘ありがとうございます」「ご要望があるか確認できますでしょうか？」「恐れ入りますが」「お手数ですが」「ご回答お願いします」「確認させてほしい」「〜してよいか？」
  - 応答は前置きやクッション言葉を一切排除し、結論・確認事項・進捗報告のみを簡潔な日本語（箇条書き等）で直接的に記述すること。
...
2. **テストコード実装方針**:
   ...
   - **インスタンス生成方針**:
     ...
     - `Moq` でも対応が困難な場合に限り、ユーザーに確認を取った上で `GetUninitializedObject` を使用すること。
