...
from refactor.constructor_arg_expend_1st import parse_arguments, refactor_constructor_calls


def test_simple_arguments():
    assert parse_arguments("arg1, arg2, arg3") == ["arg1", "arg2", "arg3"]

def test_arguments_with_spaces():
    assert parse_arguments("  arg1  ,  arg2 ,arg3 ") == ["arg1", "arg2", "arg3"]

def test_nested_parentheses():
    assert parse_arguments("roleCore, new Func(a, b), arg3") == ["roleCore", "new Func(a, b)", "arg3"]

def test_deeply_nested_parentheses():
    assert parse_arguments("a, b(c(d, e), f), g") == ["a", "b(c(d, e), f)", "g"]

def test_named_arguments():
    assert parse_arguments("canKill: true, hasTask: false") == ["canKill: true", "hasTask: false"]

def test_mixed_arguments():
    assert parse_arguments("roleCore, true, canKill: false, tab: new Func(a, b)") == ["roleCore", "true", "canKill: false", "tab: new Func(a, b)"]

def test_empty_string():
    assert parse_arguments("") == []

def test_single_argument():
    assert parse_arguments("roleCore") == ["roleCore"]

def test_empty_arguments_are_filtered():
...
def test_basic_refactoring():
    ...
    assert ": base(roleCore, true, false, true)" not in actual_output
    # 2. Check that the new arguments are present
    assert "roleCore" in actual_output
    assert "true, false, true, false" in actual_output # Original bools
    assert "true, true, true, true" in actual_output # New default bools
    # 3. Check that it's still a base call
    assert ": base(" in actual_output

def test_with_named_arguments():
    source = textwrap.dedent("""
    public class MyRole : SingleRoleBase
    {
        public MyRole(RoleCore roleCore)
    ...
    assert "true, true, true, false," in actual_output
    assert ": base(roleCore, true, hasTask: true, useVent: true)" not in actual_output

def test_preserves_other_named_args():
    source = textwrap.dedent("""
    public class MyRole : SingleRoleBase
    {
        public MyRole(RoleCore roleCore)
    ...
    assert "tab: RoleTab.Default" in actual_output
    # Check that the boolean expansion happened correctly
    assert "true, false, false, false" in actual_output
    assert ": base(roleCore, true, tab: RoleTab.Default)" not in actual_output


def test_no_change_if_already_refactored():
    source = textwrap.dedent("""
    public class MyRole : SingleRoleBase
    {
        public MyRole(RoleCore roleCore)
    ...
    """)
    assert refactor_constructor_calls(source) == source
...
def test_multiple_calls_in_file():
    ...
    assert "true, false, false, false" in result
    assert "false, true, false, false" in result
    assert ": base(roleCore, true)" not in result
    assert ": base(roleCore, false, true)" not in result

def test_skips_if_too_many_newlines():
...
def test_idempotence(code_parts):
    """
    ...
    assert refactored_once == refactored_twice

@settings(deadline=500)
@given(code=st.text(alphabet=st.characters(max_codepoint=127)))
def test_no_change_if_no_target(code):
    """
    リファクタリング対象が含まれない場合、コードは変更されない。
    """
    # : base( を含まない文字列を生成
    code_without_target = code.replace(": base(", "---")
    assert refactor_constructor_calls(code_without_target) == code_without_target

@settings(deadline=500)
@given(code_parts=csharp_code_strategy())
def test_invariance_of_surrounding_code(code_parts):
    """
    ...
    assert refactored_code.startswith(prefix)
    assert refactored_code.endswith(suffix)
...
def test_correctness_with_named_args(generated_args):
    """
...
"""
    ...
    assert match is not None, f"Could not find the argument block in the refactored code for input: base({args_str})"
    ...
    assert len(actual_bools) == 9, f"Expected 9 boolean arguments, but found {len(actual_bools)} in '{arg_block_str}'"
    assert actual_bools == expected_bool_list, f"Boolean argument mismatch for input: base({args_str})"
